...
__global__ void assign_rowidx(int *d_rowidx, int num_v) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= num_v) return;
    d_rowidx[i] = i;
}
...
void SpMMOpt::preprocess(float *vin, float *vout)
{
    ...
    if (is_sparse(num_v, num_e, feat_in)) {
        ...
        sort(perm, perm + num_v, [&](const int &x, const int &y) {
            ...
            int iter_end = min(end_x - start_x, end_y - start_y);
            ...
            for (int i = 0; i < iter_end; ++i) {
                ...
                // }
                if (cur_idx[start_x + i] < cur_idx[start_y + i]) {
                    return true;
                }
            }
        ...
        cudaMalloc((void**)&d_rowidx, num_v * sizeof(int));
        cudaMemcpy(d_rowidx, perm, num_v * sizeof(int), cudaMemcpyHostToDevice);
