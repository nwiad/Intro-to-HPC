...
__global__ void spmm_kernel_parallel(int *rowptr, int *idx, float *val, float *vin, float *vout, int num_v, int feat_in) {
    ...
    for (int kk = 0; kk < WARP_SIZE; kk++) {
        result += vin[sm_k[tid_x][kk] + j] * sm_v[tid_x][kk];
        if (kk == WARP_SIZE - 1 || sm_rowptr[tid_x][kk] != sm_rowptr[tid_x][kk + 1]) {
            atomicAdd(vout + sm_rowptr[tid_x][kk] + j, result);
            result = 0.0f;
        }
    }
}
...
void SpMMOpt::run(float *vin, float *vout)
{
    ...
    if (sparsity(num_v, num_e) < 0.00001) {
        ...
        spmm_kernel_parallel<<<grid, block>>>(d_rowptr, pad_idx, pad_val, vin, vout, num_v, feat_in);
    }
    // if the matrix is not so sparse
