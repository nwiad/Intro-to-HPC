...
__global__ void spmm_kernel_parallel(int* ptr, int* idx, int* line, float* val, float* vin, float* vout, int num_v, int feat_in) {
    ...
    for (int kk = 0; kk < WARP_SIZE; kk++) {
        result += (vin + dense_offset)[sm_k[tid_x][kk]] * sm_v[tid_x][kk];
        if (kk == WARP_SIZE - 1 || sm_rowidx[tid_x][kk] != sm_rowidx[tid_x][kk + 1]) {
            atomicAdd(vout + dense_offset + sm_rowidx[tid_x][kk], result);
            result = 0.0f;
        }
    }
}
...
void SpMMOpt::run(float *vin, float *vout)
{
    // TODO: your code
    if (sparsity(num_v, num_e) < 0.00001) {
        spmm_kernel_parallel<<<grid, block>>>(d_ptr, pad_idx, d_rowptr, pad_val, vin, vout, num_v, feat_in);
    }
