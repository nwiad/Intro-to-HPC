...
__global__ void spmm_kernel_coarse(int *ptr, int *idx, float *val, float *vin, float *vout, int num_v, int feat_in) {
    __shared__ int sm_k[BLOCK_SIZE][WARP_SIZE];
    __shared__ float sm_v[BLOCK_SIZE][WARP_SIZE];
    int tid_x = threadIdx.x >> 5;
    int tid_y = threadIdx.x & 0x1f;
...
{
    // TODO: your code
    coarsen = feat_in > 128 ? true : false;
    if (coarsen) {
...
void SpMMOpt::run(float *vin, float *vout)
{
    // TODO: your code
    if (coarsen) {
        spmm_kernel_coarse<<<grid, block>>>(d_ptr, d_idx, d_val, vin, vout, num_v, feat_in);
    }
    else {
